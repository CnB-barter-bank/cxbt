...
contract PresaleManager is Ownable, AccessManager, Pausable {
  ...
  function setRate(
    ...
    if (tokenContract == address(0)) revert EmptyToken();
    if (rate > uint256(10 ** (IERC20Metadata(tokenContract).decimals() + 2)))
  ...
  function setBonus(uint8 bonus) external onlyOwner {
    bonusPercent = 100 + uint256(bonus);
  }
  ...
  function getResultAmount(
    ...
    uint256 rate = this.rate(currency);
